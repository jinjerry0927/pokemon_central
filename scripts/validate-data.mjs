...
import { validateChampionsAbilitySnapshot } from "./lib/champions-ability-data.mjs";
...
const serebiiLearnsetsPath = path.join(
  rootDirectory,
  "data",
  "generated",
  "serebii-learnsets-m-b.json"
);
const serebiiLearnsetReviewPath = path.join(
  rootDirectory,
  "data",
  "generated",
...
const championsAbilitiesPath = path.join(
  rootDirectory,
  "data",
  "generated",
  "champions-abilities-m-b.json"
);
const usageInsightsPath = path.join(
  rootDirectory,
  "data",
  "generated",
  "usage-insights-m-b.json"
);
...
async function main() {
  ...
  const serebiiLearnsets = validateFullSerebiiLearnsetSnapshot(
    await readJson(serebiiLearnsetsPath),
    regulationRoster,
    regulationPokemon,
    pokemonMoveCandidates,
    moveCandidates
  );
  const serebiiLearnsetReview = validateSerebiiLearnsetReviewSnapshot(
  ...
  const championsAbilities = validateChampionsAbilitySnapshot(
    await readJson(championsAbilitiesPath),
    currentPokemon
  );
  const usageInsights = validateUsageInsightSnapshot(
    await readJson(usageInsightsPath),
    currentPokemon,
    moveCandidates,
    championsItems,
    championsAbilities
